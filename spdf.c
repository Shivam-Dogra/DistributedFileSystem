...
void clientHandler(int cSocket) {
   ...
   //rmfile
    ...
    strcat(completeFilePth, strrchr(flNm, '/') + 1); 
    ...
    else if (strcmp(pdfCommand, "dfile") == 0) {
        char *space_pos = strchr(pdfBfr, ' ');
        if (space_pos != NULL) {
            *space_pos = '\0'; 
            strcpy(pdfCommand, pdfBfr);
            strcpy(flNm, space_pos + 1);  
        } 
        else {
        ...
        sendfilesToDownload(cSocket, flNm);
    }
    else {
...
void sendfilesToDownload(int cSocket, const char *nameOfFl) {
    ...
    snprintf(fullFilePath, BUF_SIZE, "/home/"user"/spdf/%s", basename(nameOfFl));

    FILE *txtFile = fopen(fullFilePath, "rb");
    if (!txtFile) {
        printf("File not found on Spdf server");
        return;
    }
